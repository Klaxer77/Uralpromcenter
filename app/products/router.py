...
from app.products.schemas import SProductADD, SProductsList
from app.logger import logger

router = APIRouter(prefix="/product", tags=["Продукты"])

@router.get("/all")
async def get_all() -> list[SProductsList]:
    get_products = await ProductDAO.get_products_all()
    return get_products
...
@router.get("/search")
async def product_search(product_name: str):
